...
将任务进度摘要同步到关联的 Pull Request 评论中。

从 `$ARGUMENTS` 中解析任务 ID。如果为空，回复：
"请提供任务 ID。示例：/sync-pr TASK-20260101-120000"
然后 STOP。
...
从 task.md 提取：PR 编号、标题、current_step、status、时间戳、issue_number。

如果缺少 pr_number，回复："任务文件中未找到 PR 编号。请在 task.md 中更新 pr_number。"然后 STOP。
...
!gh pr comment {pr-number} --body "{进度摘要}"
