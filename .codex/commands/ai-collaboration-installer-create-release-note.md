...
usage: /prompts:ai-collaboration-installer-create-release-note <version> [prev-version]
...
**用法：**
- `/prompts:ai-collaboration-installer-create-release-note 3.6.3` — 自动推断上一版本为 3.6.2
- `/prompts:ai-collaboration-installer-create-release-note 3.6.3 3.6.2` — 手动指定范围
- `/prompts:ai-collaboration-installer-create-release-note 3.7.0` — x.y.0 版本：合并 3.6.x 系列的所有 release notes
...
2. **Tag 必须已存在**：运行本命令前，确保 `v<version>` 和上一版本的 tag 已创建（通常由 `/prompts:ai-collaboration-installer-release` 命令完成）
...
- **版本号格式错误**：提示正确格式并退出
- **Tag 不存在**：提示确认 tag 已创建（可能需要先执行 `/prompts:ai-collaboration-installer-release`）
...
- `/prompts:ai-collaboration-installer-release` - 执行版本发布流程（创建 tag 和发布分支）
- `/prompts:ai-collaboration-installer-commit` - 提交代码
- `/prompts:ai-collaboration-installer-create-pr` - 创建 Pull Request
