...
执行 fitlab-ai 的集成测试，包括服务启动和接口验证。

**前置条件**：需要先完成 Maven 构建，确保 `build/` 目录存在。如果还没有构建，请先运行 `/prompts:ai-collaboration-installer-test` 命令。

**用法：**
- `/prompts:ai-collaboration-installer-test-integration` - 执行集成测试流程
...
```bash
if [ ! -d "build" ]; then
    echo "错误: 构建产物不存在，请先运行 /prompts:ai-collaboration-installer-test 命令"
    exit 1
fi
```
...
1. **前置条件**：必须先运行 `/prompts:ai-collaboration-installer-test` 完成构建，或确保 `build/` 目录存在
...
- `/prompts:ai-collaboration-installer-test` - 执行完整测试流程（包含构建）
- `/prompts:ai-collaboration-installer-commit` - 提交代码变更
