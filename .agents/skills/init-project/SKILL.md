...
name: init-project
description: >
  Initialize a new project with AI collaboration infrastructure
  and project governance from ai-collaborator templates.
---
...
- **modules**: Which modules to install (default: all)
  ...
  - [x] github - Project governance (issue templates, PR template, CI,
        contributing, security, editor config, gitignore, license)

## Step 1: Locate templates

Read template files from ai-collaborator repository.
If running from within ai-collaborator repo, use current directory.
...
Copy directly (no modification needed):
- `.mailmap` -> `.mailmap`
- `.ai-workspace/` directory structure (with .gitkeep files)

### 2b: Install bilingual-selected files

Based on language setting, select the appropriate version and install:

**Task templates** (.agents/templates/):
- en: `task.md` -> `task.md`; zh-CN: `task.zh-CN.md` -> `task.md`
- Same for handoff.md, review-report.md

**Workflows** (.agents/workflows/):
- en: `feature-development.yaml` -> `feature-development.yaml`
- zh-CN: `feature-development.zh-CN.yaml` -> `feature-development.yaml`
- Same for bug-fix, code-review, refactoring

**.ai-workspace/README.md**:
- en: `README.md`; zh-CN: `README.zh-CN.md` -> `README.md`
...
- Adapt project references:
  - Replace "ai-collaborator" / "collaborator" with project_name
  - For Codex: rename `collaborator-{cmd}.md` -> `{project}-{cmd}.md`
  - For Gemini: rename `collaborator/{cmd}.toml` -> `{project}/{cmd}.toml`
- Skip `init-project` command (ai-collaborator only)
- Include `update-project` command
- Install tool README files (language-selected)
- For OpenCode: install COMMAND_STYLE_GUIDE.md (language-selected)

### 2e: Generate AGENTS.md (language-selected)

Read AGENTS.md (or AGENTS.zh-CN.md) from ai-collaborator, adapt:
- Replace project name, org, repo URL
- Remove ai-collaborator-specific content
...
**Claude Code**:
- `.claude/CLAUDE.md` - adapt from template, mark tech-stack sections with TODO
- `.claude/project-rules.md` - adapt similarly
- `.claude/settings.json` - generate with default permissions

**Codex, Gemini, OpenCode**: Adapt config files similarly
...
- Adapt project name and org references
