...
description: >
  ...
  and project governance to match the latest ai-collaborator
...
Extract: source, project, org, branchPrefix, language,
modules, files (managed/merged/ejected lists).

## Step 2: Locate template source

If source is "self" (ai-collaborator updating itself),
use current repository as template source.

Otherwise, read from `~/.ai-collaborator/`.
If `~/.ai-collaborator/` does not exist, report error and stop:
"Template source not found. Please clone it first:
